...
function validaForm_old(id,tipo,campo,obrigatorio){
	...
	if ((tipo == "FLOAT")&&(obj.value != "")) {
		//validar dados numéricos
		if (!regFLOAT.test(obj.value)){
			alert ("O campo "+ campo +" deve contar valores com no máximo duas casas decimais");
			obj.focus();
			return false;
		}
	} else
	...
	if ((tipo == "USUARIO")&&(obj.value != "")){
		if (!regUSUARIO.test(obj.value)){
			alert("Formato de nome de usuário inválido!!");
			obj.focus();
			return false;
		}
	}	
...
function validaForm(id,tipo,campo,obrigatorio){
	...
	if (args==5){//um quinto parametro foi passado
		var texto = validaForm.arguments[4];
	}
