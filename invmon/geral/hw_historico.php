<?php 
  ...
  */session_start();
	...
	$resultado = mysql_query($query) or die(TRANS('MSG_ERR_RESCUE_INFO_HW').'<BR>'.$query);
		...
		print "<TR>";
		print "<TD width='600' align='left' ><B>".TRANS('WERE_FOUND')." $linhas ".TRANS('TXT_ALTER_REG_OF_HW_FOR_EQUIP')."</B></TD>";
		...
		$execInst = mysql_query($qryInst) or die(TRANS('MSG_ERR_GET_DATA').'<BR>'.$qryInst);
		...
		print "<TR class='header'><td class='line'>".TRANS('COMPONENT')."</TD><td class='line'>".TRANS('DESCRIPTION')."</TD><td class='line'>".TRANS('COL_MODIF_IN')."</td><td class='line'>".TRANS('COL_MODIF_FOR')."</td></tr>";
		$j=0;
