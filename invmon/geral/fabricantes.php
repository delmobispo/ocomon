	...
	if ((!isset($_GET['action'])) && empty($_POST['submit'])) {
		...
		else
		{
			...
			while ($row=mysql_fetch_array($PAGE->RESULT_SQL))
			{
				...
				print "<td class='line'><a onClick=\"redirect('".$_SERVER['PHP_SELF']."?action=alter&cod=".$row['fab_cod']."&cellStyle=true')\"><img height='16' width='16' src='".ICONS_PATH."edit.png' title='".TRANS('BT_ALTER')."'></a></td>";
				print "<td class='line'><a onClick=\"confirmaAcao('".TRANS('MSG_DEL_REG')."','".$_SERVER['PHP_SELF']."', 'action=excluir&cod=".$row['fab_cod']."')\"><img height='16' width='16' src='".ICONS_PATH."drop.png' title='".TRANS('BT_REMOVE')."'></a></TD>";
