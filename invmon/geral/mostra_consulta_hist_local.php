<?php 
 ...
 $query = "SELECT c.comp_inv AS etiqueta, c.comp_inst AS instituicao, c.comp_local AS tipo_local,
		...
		NL();
		print "<TR><TD bgcolor='".TD_COLOR."'><B>".TRANS('WERE_FOUND')." <font color=red>".$linhas."</font> ".TRANS('TXT_REG_EQUI_THIS_TYPE')." </b><i>".$equip."</i><b> ".TRANS('TXT_REG_EQUI_THIS_TYPE_2')." <i><a href=".$_SERVER['PHP_SELF']."?comp_local=".$row['local_cod']."&ordena=equipamento,fab_nome,modelo,etiqueta title='".TRANS('HNT_LIST_EQUIP_CAD_IN')." ".$row['locais']."'>".$msg2."</a></i> ".$msgData.".</b></TD></TR>";
	}
	print "<br><br>";
	print "<TABLE border='0' cellpadding='5' cellspacing='0' align='center' width='100%' >";
	print "<TR class='header'><td class='line'><b>".TRANS('ASSET_TAG')."</TD><td class='line'><b>".TRANS('COL_UNIT')."</TD><td class='line'><b>".TRANS('COL_TYPE')."</TD><td class='line'>".TRANS('COL_MODEL')."</TD><td class='line'><b>".TRANS('COL_CURRENT_LOCAL')."</TD><td class='line'><b>".TRANS('MNL_CON_HIST')."</TD>";
        $i=0;
        $j=2;
	while ($row = mysql_fetch_array($resultado2)) {
		...
		print "<td class='line'><a ".$alerta." href='equipment_show.php?comp_inv=".$row['etiqueta']."&comp_inst=".$row['instituicao']."' title='".TRANS('HNT_SHOW_DATEIL_EQUIP_CAD')."'>".$row['etiqueta']."</a></TD>";
		print "<td class='line'><a ".$alerta." href='equipments_list.php?comp_inst=".$row['instituicao']."&ordena=equipamento,fab_nome,modelo,local,etiqueta' title='".TRANS('HNT_LIST_EQUIP_CAD_UNIT')." ".$row['instituicao_nome']."'>".$row['instituicao_nome']."</a></td>";
		print "<td class='line'><a ".$alerta." href='equipments_list.php?comp_tipo_equip=".$row['tipo']."&ordena=fab_nome,modelo,local,etiqueta' title='".TRANS('HNT_LIST_ALL_EQUIP_TYPE')." ".$row['equipamento']." ".TRANS('HNT_CAD_IN_SYSTEM')."'>".$row['equipamento']."</a></td>";
		print "<td class='line'><a ".$alerta." href='equipments_list.php?comp_marca=".$row['tipo_marca']."&ordena=local,etiqueta' title='".TRANS('HNT_LIST_ALL_EQUIP_MODEL')." ".$row['fabricante']." ".$row['modelo']."'>".$row['fabricante']." ".$row['modelo']."</a></td>";
		print "<td class='line'><a ".$alerta." href='equipments_list.php?comp_local=".$row['tipo_local']."&ordena=equipamento,fab_nome,modelo,etiqueta' title='".TRANS('HNT_LIST_EQUIP_CAD_IN')." ".$rowB['loc_atual'].".'>".$rowB['loc_atual']."</a></td>";
