<?php 
  ...
  */session_start();
	...
	if ((!isset($_GET['action'])) && empty($_POST['submit'])) {
		...
		else
		{
			...
			print "<TR class='header'><td class='line'>".TRANS('COL_VENDOR')."</TD><td class='line'>".TRANS('COL_PHONE')."</TD>".
				"<td class='line'>".TRANS('BT_ALTER')."</TD><td class='line'>".TRANS('BT_REMOVE')."</TD></tr>";
			...
			while ($row = mysql_fetch_array($resultado))
			{
				...
				print "<td class='line'><a onClick=\"redirect('".$_SERVER['PHP_SELF']."?action=alter&cod=".$row['forn_cod']."&cellStyle=true')\"><img height='16' width='16' src='".ICONS_PATH."edit.png' title='".TRANS('BT_ALTER')."'></a></td>";
				print "<td class='line'><a onClick=\"confirmaAcao('".TRANS('MSG_DEL_REG')."','".$_SERVER['PHP_SELF']."', 'action=excluir&cod=".$row['forn_cod']."')\"><img height='16' width='16' src='".ICONS_PATH."drop.png' title='".TRANS('BT_REMOVE')."'></a></TD>";
	...
	if ($_POST['submit'] ==  TRANS('BT_RECORD')){
		...
		$qryl = "SELECT * FROM fornecedores WHERE forn_fone='".noHtml($_POST['forn_fone'])."' and forn_nome='".$_POST['forn_nome']."'";
		...
		if (!$erro)
		{
			...
			$query = "INSERT INTO fornecedores (forn_nome, forn_fone) values ('".noHtml($_POST['forn_nome'])."','".noHtml($_POST['forn_fone'])."')";
	...
	if ($_POST['submit'] == TRANS('BT_ALTER')){
		...
		$query2 = "UPDATE fornecedores SET forn_nome='".noHtml($_POST['forn_nome'])."', forn_fone='".noHtml($_POST['forn_fone'])."' ".
...
<!--
	function valida(){
		...
		if (ok) var ok = validaForm('idTelefone','ALFAFULLESPACO','Telefone',1);
