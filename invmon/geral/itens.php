...
// 	dump ($teste);
	...
	if ((!isset($_GET['action'])) and !isset($_POST['submit'])){
		...
		print "<TR class='header'><td class='line'>".TRANS('COL_TYPE')."</TD><td class='line'>".TRANS('COL_MODEL')."</TD><td class='line'><b>".TRANS('BT_ALTER')."</b></TD><td class='line'><b>".TRANS('BT_REMOVE')."</b></TD>";
		$j=2;
	...
	if ((isset($_GET['action'])  && $_GET['action']=="incluir") && (!isset($_POST['submit']))) {
		...
		print "<B>".TRANS('NEW_RECORD','Cadastro de modelos de componentes').":<br>";
		...
		print "<TR>";
			print "<TD width='30%'  bgcolor='".TD_COLOR."'><a title='".TRANS('HNT_COMPONENT_CAPACITY')."'>".TRANS('CAPACITY').":</a></TD>";
	...
	if ((isset($_GET['action'])  && $_GET['action']=="alter") && (!isset($_POST['submit']))) {
        ...
        print "<TR>";
                print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>".TRANS('CAPACITY').":</TD>";
	...
	if ($_POST['submit']== TRANS('BT_INCLUDE')){
		...
		if (isset($_POST['item_tipo']) && isset($_POST['item_descricao'])) {
			...
			if (isset($_POST['item_capacidade']) && $_POST['item_capacidade']=="") {
				...
				$mdit_desc_capacidade = str_replace(',', '.', $_POST['item_capacidade']);
			...
			if ($achou){
				...
				$exec = mysql_query($qry) or die (TRANS('MSG_ERR_SAVE_RECORD').'<br>'.$qry);
				print "<script>mensagem('".TRANS('MSG_SUCCESS_INSERT')."'); redirect('".$_SERVER['PHP_SELF']."?tipo=".$_POST['item_tipo']."');</script>";
				}
		} else {
	...
	if ($_POST['submit'] = TRANS('BT_ALTER')){
		if (isset($_POST['item_tipo']) && isset($_POST['item_descricao'])) {
			...
			print "<script>mensagem('".TRANS('MSG_SUCCESS_EDIT')."'); redirect('".$_SERVER['PHP_SELF']."?tipo=".$_POST['item_tipo']."');</script>";
...
<script type="text/javascript">
	function valida(){
		...
		if (ok) var ok = validaForm('idCapacidade','FLOAT','Capacidade',0);
