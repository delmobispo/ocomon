  ...
  */session_start();
	...
	if ((!isset($_GET['action'])) && empty($_POST['submit'])) {
		 ...
		 else
		{
				print "<br>";
				print "<tr>";
				print "<TD width='400' align='left'><B>".TRANS('WERE_FOUND')." <font color=red>".$registros."</font> ".TRANS('TXT_SW_CAD_CONFIG_STAND')."</B></TD>";
				print "</tr>";
				print "<TR class='header'><td class='line'><b>".TRANS('COL_SOFT')."</TD><td class='line'><b>".TRANS('CATEGORY')."</TD><td class='line'><b>".TRANS('COL_LICENSE')."</TD><td class='line'><b>".TRANS('COL_NUMBER_OF_LICENSES')."</TD><td class='line'><b>".TRANS('COL_AVAILABLE')."</TD><td class='line'><b>".TRANS('BT_REMOVE')."</TD>";
		}
		...
		//print "<table class=corpo2 >";
		   ...
		   print "<tr class=".$trClass." id='linhax".$j."' onMouseOver=\"destaca('linhax".$j."','".$_SESSION['s_colorDestaca']."');\" onMouseOut=\"libera('linhax".$j."','".$_SESSION['s_colorLinPar']."','".$_SESSION['s_colorLinImpar']."');\"  onMouseDown=\"marca('linhax".$j."','".$_SESSION['s_colorMarca']."');\">";
				 ...
				 print "<td class='line'><a onClick=\"javascript:confirmaAcao('".TRANS('REMOVE')." ".$row['fab_nome']." ".$row['soft_desc']."?','".$_SERVER['PHP_SELF']."', 'action=excluir&cod=".$row['soft_cod']."')\"><img src='".ICONS_PATH."drop.png' title='".TRANS('BT_REMOVE')."'></TD>";
			  print "</TR>";
		   }
		   print "</TABLE>";
	...
	if ((isset($_GET['action'])  && ($_GET['action'] == "incluir") )&& empty($_POST['submit'])) {
		...
		print "<tr>";
			print "<td align='center' width='20%' bgcolor=".BODY_COLOR."><input type='submit' class='button' value='".TRANS('BT_RECORD')."' name='submit'></td>";
	...
	if (isset($_POST['submit']) &&  $_POST['submit']== TRANS('BT_RECORD')){
		...
		if ($_POST['software'] == -1)
			...
			$query = "insert into sw_padrao (swp_sw_cod) values ".
				"(".$_POST['software'].")";
			$resultado = mysql_query($query) or die (TRANS('MSG_ERR_SAVE_RECORD'). $query);
			if ($resultado == 0)
			{
				print $query."<br>";
				$aviso = TRANS('MSG_ERR_SAVE_RECORD'). $query;
			}
			else
			{
