<?php 
  ...
  */session_start();
	...
	if ((!isset($_GET['action'])) && empty($_POST['submit'])) {
		...
		if ($registros == 0)
		{
			print "<tr><td>".mensagem(TRANS('NO_RECORDS_FOUND'))."</td></tr>";
		}
		else
		{
			print "<tr><td class='line'>";
			print "".TRANS('THERE_IS_ARE')." <b>".$registros."</b> ".TRANS('RECORDED')."</td>";
			print "</tr>";
			print "<TR class='header'>".
					"<td class='line'>".TRANS('DESCRIPTION')."</TD>".
					"<td class='line'>".TRANS('COL_MODEL_EQUIP')."</td>".
					"<td class='line'>".TRANS('COL_QTD')."</TD>".
					"<td class='line'>".TRANS('BOX')."</TD>".
					"<td class='line'>".TRANS('COL_OBS')."</TD>".
					"<TD class='line'>".TRANS('BT_ALTER')."</TD><td class='line'>".TRANS('BT_REMOVE')."</TD>".
				"</TR>";
			$j=2;
			while ($row = mysql_fetch_array($PAGE->RESULT_SQL))
			{
				...
				print "<td class='line'><a onClick=\"redirect('".$_SERVER['PHP_SELF']."?action=alter&cod=".$row['mat_cod']."&cellStyle=true')\"><img height='16' width='16' src='".ICONS_PATH."edit.png' title='".TRANS('BT_ALTER')."'></a></td>";
				print "<td class='line'><a onClick=\"confirmaAcao('".TRANS('MSG_DEL_REG')."','".$_SERVER['PHP_SELF']."', 'action=excluir&cod=".$row['mat_cod']."')\"><img height='16' width='16' src='".ICONS_PATH."drop.png' title='".TRANS('BT_REMOVE')."'></a></TD>";
	...
	if ((isset($_GET['action'])  && ($_GET['action'] == "incluir") ) && empty($_POST['submit'])) {
		...
		print "<TR>";
		print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>".TRANS('DOCUMENT').":</TD>";
		...
		print "</TR>";
		print "<TR>";
		print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>".TRANS('BOX').":</TD>";
		print "<TD width='80%' align='left' bgcolor='".BODY_COLOR."'><INPUT type='text' name='caixa' class='text' id='idCaixa'></td>";
		print "</TR>";
		print "<tr>";
		print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>".TRANS('MSG_ASSOC_IT').":</TD>";
	...
	if ((isset($_GET['action']) && $_GET['action']=="alter") && empty($_POST['submit'])) {
		...
		print "<TR>";
                print "<TD width='20%' align='left' bgcolor='".TD_COLOR."' valign='top'>".TRANS('DOCUMENT').":</TD>";
		...
		print "<TR>";
        	...
        	print "</TR>";
		print "<TR>";
                print "<TD width='20%' align='left' bgcolor='".TD_COLOR."' valign='top'>".TRANS('BOX').":</TD>";
                print "<TD width='80%' align='left' bgcolor='".BODY_COLOR."'><INPUT type='text' class='text' name='caixa' id='idCaixa' value='".$row['mat_caixa']."'></td>";
        	print "</TR>";
		print "<TR>";
        	print "<TR>";
                print "<TD width='20%' align='left' bgcolor='".TD_COLOR."' valign='top'>".TRANS('MSG_ASSOC_IT').":</TD>".
