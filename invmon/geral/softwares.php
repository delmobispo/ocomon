  ...
  */session_start();
	...
	if ((!isset($_GET['action'])) && empty($_POST['submit'])) {
		...
		else
		{
			...
			print "<TR class='header'><td class='line'>".TRANS('COL_SOFT')."</TD><td class='line'>".TRANS('CATEGORY')."</TD><td class='line'>".TRANS('COL_LICENSE')."</TD><td class='line'>".TRANS('COL_NUMBER_OF_LICENSES')."</TD>".
				"<td class='line'>".TRANS('COL_AVAILABLE')."</TD><td class='line'>".TRANS('COL_VENDOR')."</TD><td class='line'>".TRANS('COL_NF')."</TD><td class='line'>".TRANS('BT_ALTER')."</TD><td class='line'>".TRANS('BT_REMOVE')."</TD>";
			...
			while ($row = mysql_fetch_array($resultado))
			{
				...
				print "<td class='line'><a onClick=\"redirect('".$_SERVER['PHP_SELF']."?action=alter&cod=".$row['soft_cod']."&cellStyle=true')\"><img height='16' width='16' src='".ICONS_PATH."edit.png' title='".TRANS('BT_ALTER')."'></a></td>";
				print "<td class='line'><a onClick=\"confirmaAcao('".TRANS('MSG_DEL_REG')."','".$_SERVER['PHP_SELF']."', 'action=excluir&cod=".$row['soft_cod']."&cellStyle=true')\"><img height='16' width='16' src='".ICONS_PATH."drop.png' title='".TRANS('BT_REMOVE')."'></a></TD>";
				print "</TR>";
	...
	if ((isset($_GET['action'])  && ($_GET['action'] == "incluir") )&& empty($_POST['submit'])) {
		...
		print "<BR><B>".TRANS('NEW_RECORD')."</B><BR>";
		...
		print "<tr>";
			print "<td width='20%' align='right' bgcolor=".TD_COLOR.">".TRANS('CATEGORY').":</TD>";
			print "<td width='80%' align='left' bgcolor=".BODY_COLOR.">";
	...
	if ((isset($_GET['action']) && $_GET['action']=="alter") && empty($_POST['submit'])) {
		...
		print "<BR><B>".TRANS('SUBTTL_EDIT_SOFT')."</B><BR>";
			...
			print "<tr><td bgcolor=".TD_COLOR.">".TRANS('CATEGORY')."</td><td class='line'><select name='categoria' class='select' id='idCategoria'>";
	...
	if (isset($_GET['action']) && $_GET['action'] == "excluir"){
		...
		if (mysql_num_rows($exec_1)!=0) $texto.=" de equipamentos, ";

		$sql_2 = "select * from sw_padrao where swp_sw_cod = '".$_GET['cod']."'";
		$exec_2 = mysql_query($sql_2);
		$total+=mysql_num_rows($exec_2);
		if (mysql_num_rows($exec_2)!=0) $texto.=", Softwares padrão";
	...
	if ($_POST['submit'] == TRANS('BT_RECORD'))  {
		...
		$resultado = mysql_query($qryl) or die(TRANS('MSG_ERR_GET_DATA').'<BR>'.$qryl);
	...
	if ($_POST['submit'] == TRANS('BT_ALTER')){
		...
		$resultado2 = mysql_query($query2) or die(TRANS('MSG_ERR_DATA_UPDATE').'<BR>'.$query2);
...
<script type="text/javascript">
	function valida(){
		...
		if (ok) var ok = validaForm('idCategoria','COMBO','<?php print TRANS('CATEGORY');?>',1);
