<?php 
  ...
  */session_start();
	...
	if ($linhas == 0)
	{
		...
		print "<tr><td align='left' width='80%'><a onClick= \"javascript:popup_alerta_wide('../../ocomon/geral/ticket_add.php?invTag=".$_GET['comp_inv']."&invInst=".$_GET['comp_inst']."&invLoc=".$rowLocal['comp_local']."')\">".TRANS('TO_OPEN_TICKET_TO_ASSET')."</a></td><td align='right'><input type='button' class='minibutton' value='".TRANS('LINK_CLOSE')."' onClick=\"javascript:self.close()\"</td></tr>";
		print "</table>";
		exit;
	}
	else
	{
		...
		print "<tr>";
		print "<TD width='500' align='left'><B>".TRANS('MNL_CAD_EQUIP')." ".$_GET['comp_inv'].": <font color='red'>".$linhas."</font> ".TRANS('TICKETS').":</B></TD>";
	...
	print "<TR class='header'><td class='line'>".TRANS('TICKET_NUMBER')."</TD><td class='line'>".TRANS('ISSUE_TYPE')."</TD><td class='line'>".TRANS('OPENING')."</TD><td class='line'>".TRANS('OCO_SEL_CLOSE')."</TD><td class='line'>".TRANS('STATE')."</TD>";
	...
	while ($row = mysql_fetch_array($commit))
	{
		...
		$execSubCall = mysql_query($sqlSubCall) or die (TRANS('MSG_ERR_GET_DATA').'<br>'.$sqlSubCall);
		...
		if ($regSub > 0) {
			...
			while ($rowSubPai = mysql_fetch_array($execSubCall)){
				...
				$execStatus = mysql_query($sqlStatus) or die (TRANS('MSG_ERR_GET_DATA').'<br>'.$sqlStatus);
		...
		$execImg = mysql_query($qryImg) or die (TRANS('MSG_ERR_GET_DATA'));
		...
		print "<td class='line'><a onClick= \"javascript:popup_alerta('../../ocomon/geral/ticket_show.php?popup=true&numero=".$row['numero']."')\"><font color='blue'>".$row['numero']."</font></a>".$imgSub."</TD>";
	...
	print "<table width='100%'>";
	print "<tr><td align='left' width='80%'><a onClick= \"javascript:popup_alerta_wide('../../ocomon/geral/ticket_add.php?invTag=".$_GET['comp_inv']."&invInst=".$_GET['comp_inst']."&invLoc=".$rowLocal['comp_local']."&telefone=".$invRamal."')\">".TRANS('TO_OPEN_TICKET_TO_ASSET')."</a></td><td align='right'><input type='button' class='minibutton' value='".TRANS('LINK_CLOSE')."' onClick=\"javascript:self.close()\"</td></tr>";
	print "</table>";
