  ...
  */session_start();
	...
	if ((!isset($_GET['action'])) and !isset($_POST['submit'])) {
		...
		else
		{
			...
			print "<TR class='header'><td class='line'><b>".TRANS('FIELD_TIME_MONTH')."</b></TD><td class='line'><b>".TRANS('BT_ALTER')."</b></TD><td class='line'><b>".TRANS('BT_REMOVE')."</b></TD>";
			$j=2;
			while ($row=mysql_fetch_array($resultado))
			{
				...
				print "<td class='line'><a onClick=\"redirect('".$_SERVER['PHP_SELF']."?action=alter&cod=".$row['tempo_cod']."')\"><img height='16' width='16' src='".ICONS_PATH."edit.png' title='".TRANS('BT_ALTER')."'></a></TD>";
				print "<td class='line'><a onClick=\"confirma('".TRANS('ENSURE_DEL')."?','".$_SERVER['PHP_SELF']."?action=excluir&cod=".$row['tempo_cod']."')\"><img height='16' width='16' src='".ICONS_PATH."drop.png' title='".TRANS('BT_REMOVE')."'></a></TD>";
				print "</TR>";
			}
			print "</TABLE>";
		}
	...
	if ((isset($_GET['action'])  && $_GET['action']=="incluir") && (!isset($_POST['submit']))) {
		...
		print "<B>".TRANS('NEW_RECORD','Cadastro de tempo de garantia').":<br>";
		...
		print "<td class='line'><input type='reset' name='reset' class='button' value='".TRANS('bt_cancel')."' onclick=\"redirect('".$_SERVER['PHP_SELF']."')\"></td></tr>";
	...
	if ((isset($_GET['action'])  && $_GET['action']=="alter") && (!isset($_POST['submit']))) {
		...
		print "<B>".TRANS('RECORD_EDITION').":<br>";
		...
		print "<td class='line'><input type='reset' name='reset' class='button' value='".TRANS('bt_cancel','',0)."' onclick=\"redirect('".$_SERVER['PHP_SELF']."')\"></td></tr>";
	...
	if ($_POST['submit']==TRANS('bt_cad')){
		if (isset($_POST['tempo'])){
			...
			if ($achou){
				...
				print "<script>mensagem('".TRANS('MSG_SUCCESS_INSERT')."!'); redirect('".$_SERVER['PHP_SELF']."');</script>";
			}
		} else {
	...
	if ($_POST['submit'] = TRANS('BT_ALTER')){
		if (!empty($_POST['tempo'])){
			...
			print "<script>mensagem('".TRANS('MSG_SUCCESS_EDIT')."'); redirect('".$_SERVER['PHP_SELF']."');</script>";
		} else {
			print "<script>mensagem('".TRANS('MSG_EMPTY_DATA')."'); redirect('".$_SERVER['PHP_SELF']."');</script>";
		}
	}
