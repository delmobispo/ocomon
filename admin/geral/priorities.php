				...
				<table id="table_lists" class="stripe hover order-column row-border" border="0" cellspacing="0" width="100%">
					...
					<thead>
						<tr class="header">
							...
							<td class="line "><?= TRANS('COL_DEFAULT'); ?></td>
							<td class="line color"><?= TRANS('COL_COLOR'); ?></td>
					...
					<tbody>
						...
						foreach ($resultado->fetchall() as $row) {
						    ...
						    ?>
							<tr>
								<td class="line">#<?= $row['pr_nivel']; ?></td>
								<td class="line"><?= $row['pr_desc']; ?></td>
								<td class="line"><?= transbool($row['pr_default']); ?></td>
								<td class="line"><span class="badge" style="background: <?= $row['pr_color']; ?>">&nbsp;&nbsp;&nbsp;</span></td>
		...
		if ((isset($_GET['action'])  && ($_GET['action'] == "new")) && !isset($_POST['submit'])) {
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				...
				<div class="form-group row my-4">
                    ...
                    <label for="priority_color" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('COL_COLOR'); ?></label>
					<div class="form-group col-md-10">
