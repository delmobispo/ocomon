  ...
  */session_start();
	...
	if ((!isset($_GET['action'])) and !isset($_POST['submit'])) {
		...
		else
		{
			...
			print "<TR class='header'><td class='line'><b>".TRANS('COL_TYPE')."</b></TD><td class='line'><b>".TRANS('BT_ALTER')."</b></TD><td class='line'><b>".TRANS('BT_REMOVE')."</b></TD>";
			$j=2;
			while ($row=mysql_fetch_array($resultado))
			{
				...
				print "<td class='line'><a onClick=\"redirect('tipo_equipamentos.php?action=alter&cod=".$row['tipo_cod']."')\"><img height='16' width='16' src='".ICONS_PATH."edit.png' title='".TRANS('BT_ALTER')."'></a></TD>";
				print "<td class='line'><a onClick=\"confirma('".TRANS('ENSURE_DEL')."?','tipo_equipamentos.php?action=excluir&cod=".$row['tipo_cod']."')\"><img height='16' width='16' src='".ICONS_PATH."drop.png' title='".TRANS('BT_REMOVE')."'></a></TD>";
							print "</TR>";
			}
			print "</TABLE>";
		}
	...
	if ((isset($_GET['action'])  && $_GET['action']=="incluir") && (!isset($_POST['submit']))) {
		...
		print "<td class='line'><input type='reset' name='reset' class='button' value='".TRANS('bt_cancel')."' onclick=\"redirect('tipo_equipamentos.php')\"></td></tr>";
	...
	if ((isset($_GET['action'])  && $_GET['action']=="alter") && (!isset($_POST['submit']))) {
		...
		print "<td class='line'><input type='reset' name='reset' class='button' value='".TRANS('bt_cancel','',0)."' onclick=\"redirect('tipo_equipamentos.php')\"></td></tr>";
	...
	if (isset($_GET['action']) &&  $_GET['action']=="excluir"){
		...
		$achou = mysql_num_rows($execBusca);
		if ($achou) {
