  ...
  */session_start();
	...
	if ((empty($_GET['action'])) and empty($_POST['submit'])){
        ...
        else
        {
			...
			print "<tr><td>".TRANS('ALLOW_SELF_REGISTER')."</td><td>".transbool($row['conf_user_opencall'])."</td></tr>";
...
// 		print "</td></tr>";
	...
	if ($_POST['submit'] = TRANS('BT_ALTER')){
		...
		print "<script>mensagem('".TRANS('MSG_SUCCESS_EDIT','',0)."'); redirect('configuserscreen.php');</script>";
	}
?>
