<?php 
  ...
  */session_start();
	...
	print "<BR><B>".TRANS('COL_RECTORY')."s:</B><BR>";
	...
	if ((!isset($_GET['action'])) and !isset($_POST['submit'])) {
        ...
        else
        {
                ...
                print "<TR class='header'><td class='line'>".TRANS('COL_RECTORY')."</TD><td class='line'><b>".TRANS('BT_ALTER')."</b></TD><td class='line'><b>".TRANS('BT_REMOVE')."</b></TD>";
                $j=2;
			...
			print "<td class='line'>".$row['reit_nome']."</TD>";
                        print "<td class='line'><a onClick=\"redirect('".$_SERVER['PHP_SELF']."?action=alter&cod=".$row['reit_cod']."')\"><img height='16' width='16' src='".ICONS_PATH."edit.png' title='".TRANS('BT_ALTER')."'></a></TD>";
                        print "<td class='line'><a onClick=\"confirma('".TRANS('MSG_DEL_MAJOR_SYSTEM')."','".$_SERVER['PHP_SELF']."?action=excluir&cod=".$row['reit_cod']."')\"><img height='16' width='16' src='".ICONS_PATH."drop.png' title='".TRANS('BT_REMOVE')."'></a></TD>";
                        print "</TR>";
				}
                print "</TABLE>";
        }
	...
	if ((isset($_GET['action'])  && $_GET['action']=="incluir") && (!isset($_POST['submit']))) {
		...
		print "<B>".TRANS('NEW_RECORD').":<br><a href='".$_SERVER['PHP_SELF']."'>".TRANS('TXT_LIST_GENERAL')."</a>  <br>";
		print "<form name='incluir' method='post' action='".$_SERVER['PHP_SELF']."' onSubmit='return valida()'>";
		print "<TABLE border='0' cellpadding='5' cellspacing='0' width='50%'>";
		print "<tr>";
		print "<td width='10%'bgcolor=".TD_COLOR.">".TRANS('COL_RECTORY')."</td><td class='line'><input type='text' class='text' name='descricao' id='idDesc'></td>";
		print "</tr>";
	...
	if ((isset($_GET['action'])  && $_GET['action']=="alter") && (!isset($_POST['submit']))) {
		...
		print "<tr>";
		print "<td bgcolor=".TD_COLOR."><b>".TRANS('COL_RECTORY')."</b></td><td class='line'><input type='text' class='text' name='descricao' id='idDesc' value='".$rowAlter['reit_nome']."'>";
	...
	if (isset($_GET['action']) && $_GET['action']=="excluir"){
		...
		if ($linhas!=0) {
			...
			$exec = mysql_query($qry) or die (TRANS('MSG_ERR_DATA_REMOVE'));
	...
	if ($_POST['submit']== TRANS('BT_INCLUDE')){
		if (!empty($_POST['descricao'])){
			...
			if ($achou){
				...
				$exec = mysql_query($qry) or die (TRANS('MSG_ERR_SAVE_RECORD'). $qry);
				print "<script>mensagem('".TRANS('MSG_SUCCESS_INSERT')."');window.opener.location.reload(); redirect('".$_SERVER['PHP_SELF']."');</script>";
				}
		} else {
	...
	if ($_POST['submit'] = TRANS('BT_ALTER')){
		if ((!empty($_POST['descricao']))){
			...
			$exec= mysql_query($qry) or die(TRANS('MSG_NOT_ALTER_REG'). $qry);
				...
				print "<script>mensagem('".TRANS('MSG_SUCCESS_INSERT')."'); window.opener.location.reload(); history.go(-2)();</script>";
