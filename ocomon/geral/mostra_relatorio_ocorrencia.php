...
<?php 
        ...
        if (strlen($query)>36)
        {
                ...
                $linhas_abertas = mysql_num_rows($resultado_abertas);

                $resultado_encerradas = mysql_query($query_encerradas);
                $linhas_encerradas = mysql_num_rows($resultado_encerradas);

                $resultado_aguardando = mysql_query($query_aguardando);
                $linhas_aguardando = mysql_num_rows($resultado_aguardando);

                $resultado_atendimento = mysql_query($query_atendimento);
                $linhas_atendimento = mysql_num_rows($resultado_atendimento);

                $resultado_estudo = mysql_query($query_estudo);
                $linhas_estudo = mysql_num_rows($resultado_estudo);
