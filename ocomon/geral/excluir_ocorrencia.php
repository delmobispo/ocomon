...
print "<BODY>";
	...
	if (isset($_GET['numero']) && $_SESSION['s_nivel'] ==1) {
		...
		$resultado = mysql_query($query) or die (TRANS('MSG_ERR_DATA_REMOVE').'!<BR>'.$query);
		...
		$resultado2 = mysql_query($query2) or die (TRANS('MSG_ERR_DATA_REMOVE').'!<BR>'.$query2);

		$query3 = "DELETE FROM tempo_status WHERE ts_ocorrencia = ".$_GET['numero']."";
		$resultado3 = mysql_query($query3) or die (TRANS('MSG_ERR_DATA_REMOVE').'!<BR>'.$query3);

		$query4 = "DELETE FROM doc_time WHERE doc_oco = ".$_GET['numero']."";
		$resultado4 = mysql_query($query4) or die (TRANS('MSG_ERR_DATA_REMOVE').'!<BR>'.$query4);
		
		$query5 = "DELETE FROM global_tickets WHERE gt_ticket = ".$_GET['numero']."";
		$resultado5 = mysql_query($query5) or die (TRANS('MSG_ERR_DATA_REMOVE').'!<BR>'.$query5);
		
		$query6 = "DELETE FROM lock_oco WHERE lck_oco = ".$_GET['numero']."";
		$resultado6 = mysql_query($query6) or die (TRANS('MSG_ERR_DATA_REMOVE').'!<BR>'.$query6);
		
		$query7 = "DELETE FROM ocodeps WHERE dep_pai = ".$_GET['numero']." OR dep_filho = ".$_GET['numero']."";
		$resultado7 = mysql_query($query7) or die (TRANS('MSG_ERR_DATA_REMOVE').'!<BR>'.$query7);
		
		$query8 = "DELETE FROM solucoes WHERE numero = ".$_GET['numero']."";
		$resultado8 = mysql_query($query8) or die (TRANS('MSG_ERR_DATA_REMOVE').'!<BR>'.$query8);
		
		$query9 = "DELETE FROM imagens WHERE img_oco = ".$_GET['numero']."";
		$resultado9 = mysql_query($query9) or die (TRANS('MSG_ERR_DATA_REMOVE').'!<BR>'.$query9);
